...
// We declare a type embed.FS variable to hold our email template.
...
func New(host string, port int, username, password, sender string) Mailer {
	// Initialize a new mail.Dialer isntance with the given SMTP server settings.
	// We also configure this to use 5-second timeout whenever we send an email.
...
func (m Mailer) Send(recipient, templateFile string, data any) error {
	// The ParseFS() method parses the required template file from the embedded file system.
	...
	// Same pattern to execute the "plainBody" template and store the result
	...
	// Same pattern to execute the "htmlBody" template and store the result
	...
	// header, the SetBody() method to set plain-text body, and the AddAlternative()
